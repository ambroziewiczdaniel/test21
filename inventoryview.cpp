...
// --- Implementacja InventoryView (Konstruktor i Destruktor NAJPIERW) ---
    ...
    generateInventoryDocumentButton(nullptr)
{
    ...
    // Ustawienie delegata dla kolumny z checkboxami (indeks 4)
    ...
    // Ustawienie delegata dla edytowalnej kolumny (indeks 3) w drugiej tabeli
...
// --- Implementacja metod InventoryView ---
...
void InventoryView::setProductQuery(const QString &filterText)
{
    ...
    // Pobieramy QSqlQuery z systemWarehouse, które korzysta z LocationDao
    ...
    // Nagłówki są takie same jak w WarehouseView, korzystają z tej samej struktury zapytania
    mainTableModel->setHeaderData(0, Qt::Horizontal, "ID Produktu");
    mainTableModel->setHeaderData(1, Qt::Horizontal, "Nazwa Produktu");
    mainTableModel->setHeaderData(2, Qt::Horizontal, "Całkowita ilość w magazynie"); // To jest suma z lokalizacji
    mainTableModel->setHeaderData(3, Qt::Horizontal, "Lokalizacje (Regał-Półka-Kolumna, Ilość)");
    mainTableModel->setHeaderData(4, Qt::Horizontal, "Zaznacz do inwentaryzacji"); // Kolumna na checkbox
...
void InventoryView::updateSelectedProductsTable(int row, bool checked)
{
    ...
    // actualQuantity to teraz total_actual_quantity z LocationDao
...
void InventoryView::onGenerateInventoryDocumentClicked()
{
    ...
    // InventoryDocumentGenerator to osobna klasa, która nie komunikuje się z bazą danych
    // więc nie wymaga refaktoryzacji pod kątem systemWarehouse, chyba że chcesz,
    // aby systemWarehouse zarządzał też generowaniem dokumentów.
...
// --- Implementacja InventoryCheckboxDelegate ---
// Konstruktor delegata
...
void InventoryCheckboxDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (index.column() == 4) {
        ...
        // Używamy row() jako klucza do m_checkboxStates
...
// --- Implementacja EditableQuantityDelegate ---
