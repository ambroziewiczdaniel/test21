    ...
    generatePZButton(nullptr)
{
    ...
    // Używamy bazy danych z systemWarehouse dla completera
    productCompleterModel->setQuery("SELECT name FROM products", m_system->db);
...
void PrzyjecieView::loadProductDetailsAndLocations(int productId) {
    // Pobieramy Product (dla minimalnej ilości)
    ...
    if (product) {
        ...
        delete product; // Pamiętaj o zwolnieniu pamięci

        // Pobieramy lokalizacje z LocationDao
        ...
        for (const auto& data : warehouseData) {
            if (data.id == productId) {
                if (!data.locationsSummary.isEmpty()) {
                    ...
                    locationsSummary.replace("; ", "<br>"); // Formatowanie dla czytelności
                    locationsSummary += "<br>";
                }
                foundLocations = true;
                break;
...
void PrzyjecieView::onAddProductToReceiptClicked()
{
    ...
    // Sprawdź, czy ta kombinacja produktu i lokalizacji już istnieje w modelu receiptItemsModel
    ...
    // Odśwież model completera (jeśli produkty mogą być dodawane/usuwane w innych miejscach)
    productCompleterModel->setQuery("SELECT name FROM products", m_system->db);
}
...
void PrzyjecieView::onGeneratePZClicked()
{
    ...
    // Rozpocznij transakcję na poziomie systemu
    if (!m_system->db.transaction()) {
        ...
        qDebug() << "PrzyjecieView: Błąd rozpoczęcia transakcji:" << m_system->db.lastError().text();
        return;
    }
    ...
    for (int i = 0; i < receiptItemsModel->rowCount(); ++i) {
        ...
        // LocationDao::updateProductLocationQuantity obsługuje dodawanie/aktualizowanie
        ...
        {
            QMessageBox::critical(this, "Błąd Magazynu", "Nie można zaktualizować/dodać ilości dla " + productName + " w lokalizacji: " + m_system->db.lastError().text());
    ...
    if (allUpdatesSuccessful) {
        ...
        PZDocumentGenerator generator(m_system->db); // <-- Tutaj wywołanie jest już poprawne
